...
class TextObervationProcessor(ObservationProcessor):
    ...
    def fetch_page_html(
        ...
        for node_idx in range(len(nodes["nodeName"])):
            ...
            if "SELECTED_ELEMENT" in node_attributes_str:
                return nodes["backendNodeId"][node_idx]
    ...
    def parse_accessibility_tree(
        ...
        def dfs(idx: int, obs_node_id: str, depth: int, backend_id) -> str:

            tree_str = ""
            node = accessibility_tree[idx]
            if backend_id == node['backendDOMNodeId']:
                return int(obs_node_id)
            indent = "\t" * depth
            valid_node = True
            try:
            ...
            for _, child_node_id in enumerate(node["childIds"]):
                ...
                child_depth = depth + 1 if valid_node else depth
                child_str = dfs(
                    node_id_to_idx[child_node_id], child_node_id, child_depth, backend_id
                )
                if child_str.strip():
                    if tree_str.strip():
                        tree_str += "\n"
        ...
        tree_str = dfs(0, accessibility_tree[0]["nodeId"], depth=0, backend_id=backend_id)
    ...
    def process(self, page: Page, client: CDPSession) -> str:
        ...
        if self.observation_type == "html":
            ...
            obs_node_id = self.parse_accessibility_tree(
                accessibility_tree
            )
            return obs_node_id
            ...
            backend_id = self.fetch_page_html(
                browser_info,
                page,
                client,
                current_viewport_only=self.current_viewport_only,
        ...
        (() => {
            ...
            elements.forEach(element => {
                ...
                const dataString = [
                    ...
                    altText, classList, id, textContent, isInteractable, titleText
