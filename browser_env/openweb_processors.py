...
class ImageObservationProcessor(ObservationProcessor):
    ...
    def get_page_bboxes(self, page: Page) -> str:
        ...
        (() => {
            function isElementInteractable(element) {
                ...
                if (!isClickable) {
                    ...
                    const clickableRoles = [
                        "button", "tab", "link", "checkbox", "menuitem", "menuitemcheckbox",
                        "menuitemradio", "radio"
                    ];
                ...
                if (!isClickable) {
                    const clickableTags = [
                        "input", "textarea", "select", "button", "a", "iframe", 
                        "video", "object", "embed", "details"
                    ];
            ...
            let csvContent = "ID,Element,Top,Right,Bottom,Left,Width,Height,Alt,Class,Id,TextContent,Interactable,Title\\n";
            let counter = 1;
            ...
            elements.forEach(element => {
                ...
                // Filter elements that contain 'var elem8miVxx8zArray = document.querySelectorAll'
                const textContent = element.textContent || '';
                if (/var\\s+\\w+\\s*=\\s*document\\.querySelectorAll/.test(textContent)) return;
                ...
                if (element.tagName.toLowerCase() === 'select') {
                    ...
                    elementTextContent = Array.from(element.options).map(option => option.textContent).join('; ');
                } else {
                    // For other elements, get their text content
                    elementTextContent = element.textContent || '';
                }

                elementTextContent = elementTextContent.replace(/"/g, '""'); // Escape double quotes in textContent
                elementTextContent = elementTextContent.replace(/\\s+/g, ' ').trim(); // Remove extra whitespace
                let titleText = element.getAttribute('title') || '';
                titleText = titleText.replace(/"/g, '""'); // Escape double quotes in titleText

                // Determine if the element is interactable and in interactableSelectors
                const isInteractable = isElementInteractable(element) && interactableSelectors.some(selector => element.matches(selector));
                ...
                const dataString = [
                    ...
                    altText, classList, id, elementTextContent, isInteractable, titleText
