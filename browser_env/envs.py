...
if os.getenv('PROCESSOR_SOURCE') == 'my_processors':
    from .my_processors import ObservationHandler, ObservationMetadata
elif os.getenv("PROCESSOR_SOURCE")  == "openweb_processors":
    from .openweb_processors import ObservationHandler, ObservationMetadata
else:
    print("using original processor")
    from .original_processor import ObservationHandler, ObservationMetadata
...
def parse_action(action: str) -> PlaywrightScript:
    ...
    match splitted[:2]:
        case ["goto", url]:
            assert len(splitted) == 2
            return PlaywrightScript("goto", url, time=60000)
...
class ScriptBrowserEnv(Env[dict[str, Observation], Action]):
    ...
    def setup(self, config_file: Path | None = None) -> None:
        ...
        if os.environ["DATASET"] == "webcanvas":
    ...
    def reset(
        ...
        *,
        ...
        info = {
            ...
            "is_scroll_at_bottom": observation["is_scroll_at_bottom"],
            "is_scroll_at_top": observation["is_scroll_at_top"]
        }
