...
from typing import Any, Optional, TypedDict, Union, Tuple, Dict
...
from numpy import ndarray, dtype, unsignedinteger, floating
...
class TextObervationProcessor(ObservationProcessor):
    ...
    def parse_accessibility_tree(
        ...
        def dfs(idx: int, obs_node_id: str, depth: int) -> str:
            ...
            try:
                ...
                # select_roles = ["combobox", "listbox", "menu", "menubar", "radiogroup", "tree", "grid"]
                # if role in select_roles:
                #     node_str += f" (options: {node['description']['value']})"
                # check valid
    ...
    def process(self, page: Page, client: CDPSession) -> str:
        ...
        if self.observation_type == "html":
            ...
            #                 const values = options.map(option => option.value + ": " + option.text).join(', ');
            #                 select.setAttribute('aria-description',  values);
            #             });
            #
            #                           // 处理 <menuitem> 元素
            #             const allMenuItems = Array.from(document.querySelectorAll('menuitem'));
            #             allMenuItems.forEach(menuitem => {
            #                 const value = menuitem.getAttribute('value');
            #                 const text = menuitem.textContent || menuitem.innerText;
            #                 const description = value + ": " + text;
            #                 menuitem.setAttribute('aria-description', description);
            #             });
            #
            #         }
            #     """)
            ...
            if page.url.endswith((".jpg", ".jpeg", ".png")):
                ...
                if page.url not in self.url2caption and self.captioning_fn is not None:
                    try:
                        caption = self.captioning_fn([page.url])[0].strip()
                ...
                if self.captioning_fn is not None:
                    ...
                    for image_src in image_data:
                        if image_src and (image_src.startswith(("http://", "https://", "www.")) or image_src.startswith(
                                "data:image")):
                            valid_image_data.append(image_src)
                    ...
                    for image_src in image_data:
                        try:
                            ...
                            elif not image_src.startswith(("http://", "https://", "www.")):
                    ...
                    for image_src in images_data:
                        if image_src["src"] and (image_src["src"].startswith(("http://", "https://", "www."))
                                                 or image_src["src"].startswith("data:image")):
                            valid_image_data.append(image_src)
                    images_data = valid_image_data
...
class ImageObservationProcessor(ObservationProcessor):
    def __init__(
        ...
        with open("browser_env/javascript/get_bbox.js", "r") as f:
            self.bbox_js = f.read()

    def get_page_bboxes(self, page: Page) -> str:
        """JavaScript code to return bounding boxes and other metadata from HTML elements."""

        csv_content = page.evaluate(self.bbox_js)
        # df = pd.read_csv(StringIO(csv_content), delimiter=",", quotechar='"')
        return csv_content
    ...
    def process(self, page: Page, client: CDPSession):
        ...
        if self.observation_type == "image_som":
            ...
            try:
                ...
                bbox_img, id2center, content_str, id2semantic, df =  self.draw_bounding_boxes(
                    som_bboxes,
                    screenshot_img,
                ...
                return screenshot_img, bbox_img, content_str, df, id2center
            except:
...
class ObservationHandler:
    ...
    def get_observation(
        ...
        raw_screenshot_obs, image_obs, content_str, bboxes, id2center = self.image_processor.process(page, client)
        ...
        scroll_at_bottom = is_scroll_at_bottom(page)
        scroll_at_top = is_scroll_at_top(page)
        if content_str != "":
            text_obs = content_str
        return {"text": text_obs,
                "image": np.array(image_obs),
                "raw_screenshot": np.array(raw_screenshot_obs),
                "is_scroll_at_bottom": scroll_at_bottom,
                "is_scroll_at_top": scroll_at_top,
                "bboxes": bboxes,
                "id2center": id2center}
