...
def draw_circle_on_image(image, coords, radius=9, color=(255, 0, 0)):
    """
    在给定图像上绘制一个红色圆圈。

    :param image: PIL图像对象
    :param coords: 坐标元组 (x, y)，范围从0到1
    :param radius: 圆圈的半径
    :param color: 圆圈的颜色，默认为红色 (255, 0, 0)
    :return: 带有圆圈的图像
    """
    ...
    def render(
        ...
        if render_screenshot:
            ...
            if not isinstance(coords, np.ndarray):
                if coords != "":
                    image = draw_circle_on_image(image, coords)
            byte_io = io.BytesIO()
