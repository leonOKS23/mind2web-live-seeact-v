...
if os.getenv('PROCESSOR_SOURCE') == 'my_processors':
    from browser_env.my_processors import (
        ObservationProcessor,
        TextObervationProcessor,
    )
elif os.getenv("PROCESSOR_SOURCE") == "openweb_processors":
    from browser_env.openweb_processors import (
        ObservationProcessor,
        TextObervationProcessor,
    )
else:
    print("using original processors")
    from browser_env.original_processor import (
        ObservationProcessor,
        TextObervationProcessor,
    )
...
def create_clear_action(
    ...
    pw_code: str = "",
    nth: int = 0,
    coords: npt.NDArray[np.float32] = np.zeros(2, dtype=np.float32),
) -> Action:
    action = create_none_action()
    action.update(
        {
...
def create_coordinated_action(action_str: str, value: str, coordinate) -> Action:
    action_str = action_str.strip()
    """Main function to return individual id based action"""
    if action_str.startswith("click"):
        return create_click_action(coords=coordinate)
    elif action_str.startswith("hover"):
        return create_hover_action(coords=coordinate)
    elif action_str.startswith("type"):
        return create_type_action(text=value + '\n', coords=coordinate)
    elif action_str.startswith("press"):
        return create_key_press_action(key_comb=value)
        ...
        if "http" in value:
            return create_goto_url_action(url=value)
    ...
    elif action_str.startswith("new_tab"):
        return create_new_tab_action()
    elif action_str.startswith("go_back"):
        return create_go_back_action()
    elif action_str.startswith("go_forward"):
        return create_go_forward_action()
    elif action_str.startswith("tab_focus"):
    ...
    elif action_str.startswith("page_focus"):
        match = re.search(r"page_focus ?\[(\d+)\]", action_str)
        if not match:
            raise ActionParsingError(
                f"Invalid tab_focus action {action_str}"
            )
        page_number = int(match.group(1))
        return create_page_focus_action(page_number)
    elif action_str.startswith("close_tab"):
        return create_page_close_action()
        ...
        if not match:  # some tasks don't require an answer
            ...
            answer = match.group(1)
        return create_stop_action(answer)
    elif action_str.startswith("clear"):
        return create_clear_action(coords=coordinate)
    else:
...
def create_id_based_action(action_str: str) -> Action:
    ...
    match action:
        ...
        case "clear":
            ...
            eleid = int(match.group(1))
            return create_clear_action(eleid)
