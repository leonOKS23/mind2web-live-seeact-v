...
- Respond with ONLY the JSON action object inside a single code block (```).
- Do not include any text outside the code block.
- If the current URL is `about:blank` or you are not on the needed site, prefer `goto [url]` instead of opening new tabs.

## Homepage:
Do NOT use `homepage.com` for WebCanvas tasks unless the task explicitly requires it.
If the task involves a specific site, always prefer `goto [url]` to that site.
