...
6. If you want to select an option in a dropdown list, you can `click` to see the option firstly and use `type` to select the option. for example:
```
{{
...
```
7. If you have finished a task, please output the action `stop` and add the answer to the value field.
```
{{
    "Element Description": "",
    "Action": "stop",
...
```
8. If you want to go back to the previous page, you can use the `go_back` like:
```
{{
    "Element Description": "",
    "Action": "go_back",
...
```
9. Your element descriptions should describe the position/function/color... of the target element. For example, "the search bar on the top right of the website under the yellow logo".
...
10. Your action can be failed, please carefully check whether the previous action is successfully conducted.
11. Your "Element Description" MUST be detailed including color, position, function .etc.
12. You need to first analyze the task to determine which website you should use. Then, use Google to navigate to the appropriate website""",
