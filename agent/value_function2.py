...
def evaluate_success(screenshots: list[Image.Image], actions: list[str], current_url: str, last_reasoning: str,
    ...
    """Compute the valueva of a state using the value function.
...
If the agent repeat the actions and there is no significant progress, the answer for on the right track is "no".
If the agent have completed the intermediate steps of the target task but not entirely finish the target task, the answer for on the right track is "yes".
...
"""
        ...
        try:
            ...
            if 'success' in pred.lower():
                ...
                if 'yes' in on_path.lower():
                    ...
                    score = 0.0
                    negative_responses.append(r.message.content)
    ...
    if should_log:
        ...
        print('=' * 30)
    return score, negative_responses
