...
def get_captioning_fn(
    ...
    if "blip2" in model_name:
        ...
        def caption_request(
                ...
                server=os.environ["LOCAL_CAPTION_SERVER"],
        ) -> List[str]:
            import requests
            import os
            ...
            for i, image in enumerate(images):
                if image.startswith("http://") or image.startswith("https://"):
                    continue
                elif image.lower().endswith(("png", "jpg", "jpeg", "webp", "gif")):
                    images[i] = os.path.join(current_directory, image)
                elif image.startswith("data:image/"):
                    continue
                else:
                    raise ValueError(
                        "Image URLs must start with 'http://' or 'https://' or end with 'png', 'jpg', 'jpeg', 'webp', or 'gif'")
            if prompt is None:
                # Perform VQA
                prompt = [
                    "Please directly describe the image in detail, especially the colors, type, and names, among other relevant information of each item." for _ in range(len(images))
                ]
                data = {
                    "image_urls": images,
                    "questions": prompt,
