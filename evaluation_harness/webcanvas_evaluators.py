...
def parse_reference_evaluation(task):
    """Read labeled data"""
    task_name = task["task"]
    ...
    for i, evaluation in enumerate(evaluation_data):
        ...
        if "url" in match_function:
            ...
            try:
                ...
                exit(1)
    return [task_name, task_name_id, reference_task_length, reference_evaluate_steps]
...
def evaluate_with_webcanvas(page, selector, target_value, evaluate_steps, reference_evaluate_steps):

    if target_value != "None":
        element_value = target_value
    else:
        element_value = selector # selector is input coord. old = .text_content()
    evaluate_steps, match_result = step_evaluate(page=page, evaluate_steps=evaluate_steps, input_path=selector, element_value=element_value)
    total_step_score = 0
    for evaluate in evaluate_steps:
